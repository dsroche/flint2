...
void
_fmpz_poly_taylor_shift_multi_mod_openmp(fmpz * poly, const fmpz_t c, slong len)
{
...
void
fmpz_poly_taylor_shift_multi_mod_openmp(fmpz_poly_t g, const fmpz_poly_t f, const fmpz_t c)
{
    if (f != g)
        fmpz_poly_set(g, f);

    _fmpz_poly_taylor_shift_multi_mod_openmp(g->coeffs, c, g->length);
}
