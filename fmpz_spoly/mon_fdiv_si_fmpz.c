...
void
fmpz_spoly_mon_fdiv_si_fmpz(fmpz_spoly_t res, const fmpz_spoly_t poly,
                          slong c, const fmpz_t e)
{
    ...
    else
    {
        ...
        for(i = 0; i < t2; i++) 
        {  
            fmpz_sub(res->expons + i, poly->expons + i, e);
        }

        _fmpz_vec_scalar_fdiv_q_si(res->coeffs, poly->coeffs, t2, c);
