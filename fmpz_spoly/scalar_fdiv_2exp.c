...
void
fmpz_spoly_scalar_fdiv_2exp(fmpz_spoly_t res, const fmpz_spoly_t poly,
                          ulong exp)
{
    if (poly->length == 0)
    {
    ...
    if (res == poly)
    {
      ...
      fmpz_spoly_scalar_fdiv_2exp(temp, poly, exp);
    ...
    else
    {
      ...
      _fmpz_vec_scalar_fdiv_q_2exp(res->coeffs, poly->coeffs, poly->length, exp);
