...
main(void)
{
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_spoly_init(b);
        fmpz_spoly_init(c);
        fmpz_spoly_randtest(b, state, n_randint(state, 50), d, 50);
        fmpz_spoly_randtest(c, state, n_randint(state, 50), e, 50);

        fmpz_spoly_mul_classical(a, b, c);
        fmpz_spoly_mul_classical(b, b, c);
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_spoly_mul_classical(a, b, c);
        fmpz_spoly_mul_classical(c, b, c);
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_spoly_init(c);
        fmpz_spoly_init(d);
        fmpz_spoly_randtest(b, state, n_randint(state, 50), e, 50);
        fmpz_spoly_randtest(c, state, n_randint(state, 50), f, 50);
        fmpz_spoly_randtest(d, state, n_randint(state, 50), g, 50);

        fmpz_spoly_mul_classical(a1, b, c);
        fmpz_spoly_mul_classical(a2, b, d);
        fmpz_spoly_add(a1, a1, a2);

        fmpz_spoly_add(c, c, d);
        fmpz_spoly_mul_classical(a2, b, c);
