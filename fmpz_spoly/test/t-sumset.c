...
main(void)
{
    ...
    for (i = 0; i < 100 * flint_test_multiplier(); i++)
    {
        ...
        fmpz_init(d);
        fmpz_init(e);
        fmpz_randtest_unsigned(d, state, 30);
        fmpz_randtest_unsigned(e, state, 30);
        ...
        if (!result)
        {
            /* uncomment to print failure details
            flint_printf("FAIL:\npoly_1: ");
        ...
        fmpz_spoly_clear(c);
        fmpz_clear(d);
        if(len1 > 0) _fmpz_vec_clear(vec1, len1);
        if(len2 > 0) _fmpz_vec_clear(vec2, len2);
        fmpz_clear(e);
    }

    FLINT_TEST_CLEANUP(state);

    if (nfail == 0) flint_printf("PASS\n");
    else if (nfail * 100 < total)
    {
        flint_printf("PASS (%d/%d failures tolerated)\n", nfail, total);
    }
    else
    {
        flint_printf("FAIL: %d/%d failures\n", nfail, total);
        abort();
    }
    return 0;
}
