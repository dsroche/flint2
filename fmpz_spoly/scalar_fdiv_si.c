...
void
fmpz_spoly_scalar_fdiv_si(fmpz_spoly_t res, const fmpz_spoly_t poly,
                          slong c)
{
    if (c == 0)
    {
    ...
    if (res == poly)
    {
      ...
      fmpz_spoly_scalar_fdiv_si(temp, poly, c);
    ...
    else
    {
      ...
      _fmpz_vec_scalar_fdiv_q_si(res->coeffs, poly->coeffs, poly->length, c);
