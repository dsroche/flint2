...
void _fmpz_spoly_normalise(fmpz_spoly_t poly)
{
    ...
    for (; cur < poly->length; ++cur) 
    {
        ...
        else if (i+1 < cur) 
        {
            if (nfree == 0)
            {
                ...
                _fmpz_spoly_vec_shift(poly, i+1, cur, 1);
                poly->coeffs[i+1] = temp[0];
                poly->expons[i+1] = temp[1];
            }
            else 
            {
                _fmpz_spoly_vec_shift(poly, i+1, cur-nfree, 1);
    ...
    _fmpz_spoly_set_length(poly, poly->length - nfree);
    nfree = 0;
    ...
    while (cur < poly->length) {
        ...
        start = cur;
        for (; cur < poly->length && ! fmpz_is_zero(poly->coeffs + cur); ++cur);
        _fmpz_spoly_vec_shift(poly, start, cur, -nfree);
    }
