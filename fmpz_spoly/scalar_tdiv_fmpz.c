...
void
fmpz_spoly_scalar_tdiv(fmpz_spoly_t res, const fmpz_spoly_t poly,
                          const fmpz_t c)
{
    if (fmpz_is_zero(c))
    {
    ...
    if (res == poly)
    {
      ...
      fmpz_spoly_scalar_tdiv(temp, poly, c);
    ...
    else
    {
      ...
      _fmpz_vec_scalar_tdiv_q_fmpz(res->coeffs, poly->coeffs, poly->length, c);
