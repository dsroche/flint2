...
void 
_fmpz_spoly_add(fmpz * res_c, fmpz * res_e, slong * res_len, const fmpz * poly1_c, 
...
void
fmpz_spoly_add(fmpz_spoly_t res, const fmpz_spoly_t poly1,
    const fmpz_spoly_t poly2)
{
    ...
    if (poly1 == res || poly2 == res) 
    {
        ...
        _fmpz_spoly_add(temp->coeffs, temp->expons, &temp->length, poly1->coeffs,
    ...
    {
        fmpz_spoly_scalar_mul_ui(res, poly1, 2);
    }
    else if (poly1->length == 0)
    {
        fmpz_spoly_set(res, poly2);
    }
    ...
    {    
        ...
        _fmpz_spoly_add(res->coeffs, res->expons, &res->length, poly1->coeffs, 
                poly1->expons, poly1->length, poly2->coeffs, poly2->expons, poly2->length);
    }
}
