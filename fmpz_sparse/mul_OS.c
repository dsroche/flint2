...
fmpz_sparse_sumcheck(fmpz ** res, const fmpz_sparse_t poly1, const fmpz_sparse_t poly2)
{
  ...
  for(i = 0; i < poly1->length; i++)
  {
    ...
    {
      fmpz_mul(test + j + i*poly2->length, poly1->expons + i, poly2->expons + j);
    }
  }
...
fmpz_sparse_sumset(fmpz * res, flint_rand_t state, const fmpz_sparse_t poly1, const fmpz_sparse_t poly2) 
{
  ...
  for(i = 0; i < FLINT_MAX(22.1807097779182499013514, log(R)/0.693147180559945309417232121458 + 1); i++)
  {
    ...
    flint_printf("loooooooop\n");
  }
  
  flint_printf("here as well\n");

  /* h_1 and h_2 */
  fmpz_mul_si(L, degree, 8);
  ...
  /*TODO keep two indeces for h1 and h2 and merge them (if they match terms
   * perform this arithmetic and add to sumset otherwise ignore the extra term
   * move past the larger term when skipping) */
  i = 0;
  R = 0;
  len = 0;
  ...
  while(i < h_1->length && R < h_2->length)
  {
    ...
    {
      fmpz_cdiv_q(res + len, h_2->coeffs + R, h_1->coeffs + i);
      fmpz_sub_ui(res + len, res + len, 1);
      fmpz_cdiv_q(res + len, res + len, L);
      i++;
      R++;
      len++;
    }
